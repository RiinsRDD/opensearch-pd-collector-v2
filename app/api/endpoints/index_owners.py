...
async def create_index_owner(
    owner: IndexOwnerCreate, 
    db: AsyncSession = Depends(get_db),
...
async def update_index_owner(
    owner_id: int, 
    owner: IndexOwnerUpdate, 
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_admin)
):
