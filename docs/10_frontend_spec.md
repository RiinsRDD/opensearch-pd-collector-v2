...
└── src/
    ...
    │   │   ├── GlobalExceptions.tsx   # Глобальные исключения
    │   │   ├── IndexExceptions.tsx    # Исключения индексов
    │   │   ├── IndexOwnersList.tsx    # Владельцы (Jira)
    │   │   ├── PdnRegexList.tsx       # Регулярки ПДн
...
- **Владельцы (Jira)** (`IndexOwnersList.tsx`): управление маппингом индексов на конкретных ответственных сотрудников (ФИО), URL IT-системы в CMDB и номер Тех. долга для заведения задач в Jira.
