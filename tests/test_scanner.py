...
class MockOpenSearchClient:
    async def search_after_generator(self, index_pattern, max_docs=None, **kwargs):
...
def test_calculate_cache_key():
    scanner = ScannerService(MockOpenSearchClient())
    extra = {}
    key1 = scanner._calculate_cache_key("idx*", "user.phone", "phone", "structured_key", "phone", extra)
    key2 = scanner._calculate_cache_key("idx*", "user.phone", "phone", "structured_key", "phone", extra)
    assert key1 == key2

    key3 = scanner._calculate_cache_key("idx*", "user.email", "email", "structured_key", "email", extra)
    assert key1 != key3
